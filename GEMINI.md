...
## Branching & Pull Request Workflow Guidelines
- **Develop Branch Staging & Manual PR Trigger**: When completing a unit of work, commit and push changes **only to the `develop` branch**. Do **NOT** automatically merge `develop` into `main`. Wait for the user to explicitly ask to create a Pull Request or merge `develop` into `main`.
